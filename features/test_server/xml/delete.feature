Feature: Removing user
  ...
  and then I would like to remove it.
    ...
    When I send "DELETE" request to "{{.MY_APP_URL}}/users/{{.USER_ID}}?format=xml" with body and headers:
    """
    {
        "body": {},
        "headers": {
    ...
    """
    Then the response body should have format "XML"
  ...
  Scenario: Unsuccessful attempt to remove not existing user
    ...
    When I send "DELETE" request to "{{.MY_APP_URL}}/users/{{.NOT_EXISTING_USER_ID}}?format=xml" with body and headers:
    """
    {
        "body": {},
        "headers": {
    ...
    And the "XML" node "//Error" should match regExp "could not find in database user of id .*"
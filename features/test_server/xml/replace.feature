Feature: Replacing single user account.
  ...
  and then I would like to obtain this account to prove successful update.
    ...
    When I send "PUT" request to "{{.MY_APP_URL}}/users/{{.USER_ID}}?format=xml" with body and headers:
    """
    {
        "body": {
  ...
  I would like to be not able to modify not existing account
    ...
    When I send "PUT" request to "{{.MY_APP_URL}}/users/{{.NOT_EXISTING_USER_ID}}?format=xml" with body and headers:
    """
    {
        "body": {
            "firstName": "{{.RANDOM_FIRST_NAME}}",

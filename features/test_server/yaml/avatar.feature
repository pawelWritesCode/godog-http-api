Feature: Send avatar file using multipart/form-data in HTTP(s) request.
  ...
  Scenario: Successfully send avatar
    ...
    And the "YAML" node "$.avatar" should be "string" of value "{{.RANDOM_AVATAR_NAME}}.gif"
  ...
  Scenario: Unsuccessful attempt to add avatar for user account
    ...
    Given I prepare new "POST" request to "{{.MY_APP_URL}}/users/{{.USER_ID}}/avatar?format=yaml" and save it as "AVATAR_REQUEST"
